...
| `3` | 2026-06-07 | TikTok native captions on the WS (new `caption` event), per-chat `language` + `messageUuid`, per-gift `transactionId` + `senderUserId` + `relationship`, member `entrySource` / `entryAction` / `entryType` (creator-analytics signals), 34 brand-new event types across Tier 1 / Tier 2 / Tier 3 / Tier 4 rollouts (`linkMicOpponentGift`, `imDelete`, `goalUpdate`, `privilegeAdvance`, `commentTray`, `linkLayer`, `link`, `giftPanelUpdate`, `gameServerFeature`, `anchorToolModification`, `shareRevenueNotice`, `viewerPicksUpdate`, `fanTicket`, `giftDynamicRestriction`, `inRoomBanner`, `battlePunishFinish`, `battleNotice`, `hostBoard`, `poll`, `competition`, `streamStatus`, `battleGameplay`, `aiSummary`, `giftGallery`, `cohostLayoutUpdate`, `fansEvent`, `linkScreenChange`, `roomSticker`, `bottomMessage`, `oecLiveShopping`, `rankText`, `accessRecall`, `unauthorizedMember`, `guestShowdown`), battle `extraHostUserIds` + `layoutSubtype`, battleArmies `transactionId` dedup. |
